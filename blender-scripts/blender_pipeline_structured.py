...
def run_morpho_dynamic_pipeline():
    ...
    # Allow external callers (e.g. FastAPI blender_runner.py) to pass per-job
    # directories via environment variables — falls back to the original
    # script-relative paths so standalone MP usage is unchanged.
    ...
    try:
        # Texture baking in Paper Model requires Cycles (Eevee does not support baking and will crash)
        ...
        class UnfoldProperties:
            ...
            sticker_width = 0.005
            paper_thickness = 0.0
            angle_epsilon = 0.5  # ~28 deg angle threshold to keep major panels (bodice, skirt, sleeves) intact
            texture_type = 'NONE'  # Texture baking is unsupported in headless mode
            nesting_method = 'PACK_ISLANDS'
            style = ExportStyle()

        props = UnfoldProperties()
        
        # Instantiate the Unfolder engine directly
        print(f"[EXPORT]   Initializing ARAP structured unfolder for {len(obj.data.polygons)} faces...")
        unfolder = unfolder_module.Unfolder(obj)
        unfolder.do_create_uvmap = False
        
        # Prepare: keep panels contiguous without slicing across page boundaries
        cage_size = Vector((props.output_size_x * 2.0, props.output_size_y * 2.0))
        print(f"[EXPORT]   Computing optimal contiguous garment layout...")
